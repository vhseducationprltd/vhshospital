...
public class DISCHARGE extends javax.swing.JFrame {
  ...
  private void initComponents() {
    ...
    OTAMOUNT.addPropertyChangeListener(
    ...
    if (evt.getKeyCode() == 10) {
      ...
      try {
        ...
        if (res.next()) {
          ...
          result =
            statement.executeQuery(
              "SELECT CHARGES  FROM VHSHOSPITAL.WARDS WHERE WARDNAME='" +
              wardName +
              "'"
            );
          while (result.next()) {
          ...
          dd();
          WARDCHARGES.setText(
            Integer.toString(bedCharges * Integer.parseInt(NOOFDAYS.getText()))
          );
        } else {
  ...
  void dd() {
    try {
      String start =
        ((JTextField) ADMITDATE.getDateEditor().getUiComponent()).getText();
      String end =
        ((JTextField) DISCHARGEDATE.getDateEditor().getUiComponent()).getText();
  ...
  private void RSUBMIT2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RSUBMIT2ActionPerformed
    if (pid != 0) {
      ...
      String queryDischarge =
        "INSERT INTO doc_" +
        did +
        ".DISCHARGE(PID ,PATIENTNAME ,ADMITDATE ,DISCHARGEDATE ,NODAYS ,DRNAME ,WARDNAME ,WARDCHARGES ,OPERATIOINAMT ,EXTRAAMT ,TOTALAMOUNT ,PRESCRIPTION ,MEDICINES ,NEXTVISIT ,DATE ,TIME)VALUES(" +
        pid +
        "," +
        "'" +
        FullName +
        ...
        ")";
      String queryDeath =
        "INSERT INTO VHSHOSPITAL.DEATHS(PID ,PATIENTNAME ,ADMITDATE ,DISCHARGEDATE ,NODAYS ,DRNAME ,WARDNAME ,WARDCHARGES ,OPERATIOINAMT ,EXTRAAMT ,TOTALAMOUNT ,DEATHREASON,DEATHDATE,DEATHTIME ,DATE ,TIME)VALUES(" +
        pid +
        "," +
        "'" +
        FullName +
        ...
        "'" +
        ")";
      String queryDeleteAdmit =
        "DELETE FROM DOC_" + did + ".ADMIT WHERE PID=" + pid;
      String mainQuery = "";
      try {
  ...
  private void OTAMOUNTPropertyChange(java.beans.PropertyChangeEvent evt) {}                                                                                 
  ...
  private void getDischargeData() {
    ...
    String nextvisit =
      ((JTextField) NEXTVISIT.getDateEditor().getUiComponent()).getText();
    try {
    ...
    String deathdate =
      ((JTextField) DEATHDATE.getDateEditor().getUiComponent()).getText();
    try {
